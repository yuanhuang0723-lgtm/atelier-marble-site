...
export default function AdminPagesPage() {
  ...
  async function archive(id: string) { const result = await getSupabaseBrowserClient().from("site_pages").update({ status: "archived", updated_at: new Date().toISOString() }).eq("id", id); if (result.error) setMessage(result.error.message); else await loadRows(); }
